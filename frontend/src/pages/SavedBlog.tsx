...
import { useRecoilStateLoadable, useSetRecoilState,  useRecoilValueLoadable, useRecoilValue } from "recoil";
  ...
  if(savedBlogs.state === "loading"){
    ...
    return(
      ...
      {loader && <Loader></Loader>}
        ...
        <h1 className="px-4 text-xl font-bold text-indigo-500 underline">Saved blog's</h1>
        <BlogCards></BlogCards>
      ...
      <>
        {
          savedBlogs.contents.map((blg:any)=>{
            ...
            <div className="content w-96 md:w-2/3  flex flex-col gap-2 items-start pl-2 md:pl-0">
              ...
              <p className="h-8 md:h-24 overflow-hidden w-full text-sm flex items-center">{blg.blog?.content}</p>
              ...
              <ReadBlog prop={{id:blg.id}}></ReadBlog>
              </div>
            </div>
            <div className="image h-36 w-full md:h-48 md:w-48 md:rounded-full bg-slate-500 relative">
              <img src={"https://tse2.mm.bing.net/th?id=OIP.NfYZ6yGINT_NzZDDzEwskQHaEK&pid=Api&P=0&w=300&h=300"} alt="blogimgx" className="h-full w-full md:rounded-full object-center object-cover" />
            </div>
          </div>
            )
